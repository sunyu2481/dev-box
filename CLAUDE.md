...
- **容器 ENV 到不了 SSH 会话，须经 `/etc/environment` 中转**：容器 ENV 只被 PID 1 的后代继承，而 SSH 登入的会话不在这条链上——sshd 由启动脚本经 `sudo` 拉起，sudoers 的 `Defaults env_reset` 会清掉自定义变量。因此 compose 注入的 `CHROMIUM_MANAGER_URL` 等值在 SSH 会话（含其中运行的 VS Code Server 与各类 agent）里为空，典型症状是 `curl "$CHROMIUM_MANAGER_URL/agent/acquire"` 打到空地址、HTTP 000。`start-with-gateway` 的 `export_env_for_ssh` 把白名单变量写入 `/etc/environment`，靠 `UsePAM yes` + `/etc/pam.d/sshd` 中的 `pam_env.so` 注入会话来打通。改动约束：只导出白名单变量（不要写入 `SSH_PUBLIC_KEY` 这类敏感值，该文件全用户可读）；写入块用 `# >>> dev-box runtime env >>>` 标记包裹并在每次启动时先删后写，保证容器重启不堆积；含换行或双引号的值直接跳过，`pam_env` 只认简单的 `KEY=VALUE`，格式错误会导致整个文件解析异常。
