...
- 在镜像内验证主要工具链：
  ```bash
  docker run --rm dev-box:local bash -lc 'node --version && pnpm --version && playwright --version && python --version && uv --version && go version && java -version && mvn --version && gradle --version && docker --version && docker buildx version && docker compose version && gh --version'
  ```
